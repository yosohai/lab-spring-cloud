...
//            smallerBase64 = smallerBase64.replaceAll("\n", "").replaceAll("\r", "");//删除 \r\n
    ...
    @RequestMapping(value = "/thumbnails2")
    public void thumbnails2(@RequestParam("thumb") String thumb, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (StrUtil.isBlank(thumb)) {
            throw new Exception("上传失败，上传图片数据为空");
        }
        ...
        try (ByteArrayInputStream inputStream = new ByteArrayInputStream(bytes); ByteArrayOutputStream outputStream = new ByteArrayOutputStream();) {
            ...
            FileUtils.writeByteArrayToFile(new File(SAVE_PATH_IMAGES + tempFileName), ImageUtils.bufferedImageToBytes(image));
            /*--------------------上面保存到磁盘，下面返回缩略图流数据-----------------------------------*/
            ImageIO.write(image, "png", outputStream);//写入流中
